...
class _UploadDocumentsScreenState extends State<UploadDocumentsScreen> {
  final carDocController = Get.put(CarDocController());

  final carLoanController = Get.put(CarLoanController()) ; 
  ...
  Future<void> _pickImage(String docType) async {
    ...
    if (result != null && result.files.single.path != null) {
      ...
      carDocController.setDocument(docType, filePath);
    }
  }
...
Widget build(BuildContext context) {
  return Scaffold(
    ...
    body: Obx(() {
      ...
      if (carDocController.isLoading.value) {
        return Center(
          child: Column(
      ...
      return SingleChildScrollView(
       ...
       children: [
  Text('Your selected loan type : ${carLoanController.selectedLoanType}', style: Theme.of(context).textTheme.titleLarge),
  SizedBox(height: 10),

  
  // Conditional Fields based on Loan Type
  if (carLoanController.selectedLoanType.value == 'New Car Purchase') ...[
    _buildUploadCard(context, 'Car Loan Quotation', 'car_loan_quotation'),
  ],

  if (carLoanController.selectedLoanType.value == 'Car Refinance') ...[
    _buildUploadCard(context, 'RC', 'RC'),
    SizedBox(height: 10),
    _buildUploadCard(context, 'Insurance', 'insurance'),
    if (carLoanController.freeOrFinance == "Finance") ...[
      SizedBox(height: 10),
  ...
  if (carLoanController.selectedLoanType.value == 'Used Car Purchase') ...[
    _buildUploadCard(context, 'RC', 'RC'),
    SizedBox(height: 10),
    _buildUploadCard(context, 'Insurance', 'insurance'),
  ],

  if (carLoanController.selectedLoanType.value == 'Car Loan Transfer') ...[
    _buildUploadCard(context, 'RC', 'RC'),
    SizedBox(height: 10),
    _buildUploadCard(context, 'Insurance', 'insurance'),
    if (carLoanController.freeOrFinance == "Finance") ...[
  ...
  Center(
    child: ElevatedButton(
      ...
      onPressed: () async {
        await carDocController.uploadAllDocuments();
        if (carDocController.isError.value) {
...
class DocumentUploadCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final CarDocController loanController = Get.find<CarDocController>();
...
class CarDocController extends GetxController {
  ...
  Future<String> uploadDocumentToFirebase(String docType, String filePath) async {
    ...
    try {
      ...
      Reference storageRef = FirebaseStorage.instance.ref().child('carLoanDocuments/$docType/$fileName');
